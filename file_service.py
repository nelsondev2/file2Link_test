...
class FileService:
    ...
    def add_user(self, user_id, user_first_name=""):
        """Agregar nuevo usuario (solo para crear directorio)"""
        ...
        try:
            ...
            for user_dir in os.listdir(BASE_DIR):
                ...
                if os.path.isdir(user_path) and user_dir.isdigit():
                    ...
                    packed_path = os.path.join(user_path, "packed")
                    
                    if os.path.exists(downloads_path):
                        total_files_downloads += len([f for f in os.listdir(downloads_path) if os.path.isfile(os.path.join(downloads_path, f))])
                    
                    if os.path.exists(packed_path):
                        total_files_packed += len([f for f in os.listdir(packed_path) if os.path.isfile(os.path.join(packed_path, f))])
