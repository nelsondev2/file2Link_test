...
class DownloadManagerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("TURBO DOWNLOADER REAL-TIME")
    ...
    def get_robust_file_size(self, url, session):
        """
        Intenta obtener el tamaño del archivo usando 3 métodos robustos.
        1. HEAD Request
        2. GET Request parcial (Range)
        """
        # Método 1: HEAD
        try:
        ...
        try:
            ...
            if 'content-range' in resp.headers:
                ...
                # Formato esperado: bytes 0-0/10485760
                if '/' in cr:
                    size = int(cr.split('/')[-1])
                    if size > 0: 
                        resp.close()
    ...
    def download_engine(self, url, filepath, item_id):
        ...
        # Usamos session para mejorar la eficiencia de conexión
        ...
        while retry_count < max_retries:
            try:
                ...
                # Paso Previo: Detección Robusta de Tamaño
                total = self.get_robust_file_size(url, session)
                self.update_status(item_id, "size", self.format_size(total) if total > 0 else "---")

                self.update_status(item_id, "status", f"⬇️ Conectando ({retry_count+1})", "descargando")
                
                # Descarga Real
                with session.get(url, stream=True, timeout=20) as r:
                    r.raise_for_status()
                    
                    # Método 3: Análisis Progresivo (Si los métodos anteriores fallaron)
                    if total <= 0:
                        total_header = r.headers.get('content-length')
                        if total_header:
                            total = int(total_header)
                            self.update_status(item_id, "size", self.format_size(total))
                    ...
                    with open(filepath, 'wb') as f:
                        for chunk in r.iter_content(chunk_size=16384): 
                            ...
                            if chunk:
                                ...
                                if current_time - last_ui_update > 0.1 or downloaded == total:
                                    self.refresh_ui(item_id, downloaded, total, start_time)
                ...
                if retry_count >= max_retries:
                    if not self.cancel_flags.get(item_id, False):
                        self.update_status(item_id, "status", "❌ Error Red", "error")
                else:
                    time.sleep(1)
    ...
    def refresh_ui(self, item_id, downloaded, total, start_time):
        ...
        if total > 0:
            ...
            progress_str = f"📥 {self.format_size(downloaded)}"
