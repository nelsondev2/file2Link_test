...
async def handle_file(client: Client, message: Message) -> None:
    ...
    if user_id not in _processing:
        # ensure_future agenda la coroutine en el loop que Pyrogram ya está
        # usando, independientemente del hilo desde el que se llame
        asyncio.ensure_future(_process_queue(client, user_id))
